...
import {
  getOrganization, updateOrganizationSettings,
  listMembers, setMemberStatus, assignRole, revokeRole,
...
export const Route = createFileRoute("/_authenticated/org/$orgId/settings")({
  head: () => ({ meta: [{ title: "Organization settings — Teraji" }, { name: "robots", content: "noindex" }] }),
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">/{org.data.slug} · {org.data.status}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        <Tabs defaultValue="general">
          ...
          <TabsContent value="general"><GeneralTab orgId={orgId} org={org.data} onSaved={() => org.refetch()} /></TabsContent>
          <TabsContent value="branding"><BrandingTab orgId={orgId} org={org.data} onSaved={() => org.refetch()} /></TabsContent>
          <TabsContent value="terminology"><TerminologyTab orgId={orgId} org={org.data} onSaved={() => org.refetch()} /></TabsContent>
      ...
      <CardContent>
        <form className="grid gap-4 sm:grid-cols-2" onSubmit={(e) => { e.preventDefault(); mut.mutate(); }}>
          ...
          <div className="grid gap-2">
            ...
            <Input value={timezone} onChange={(e) => setTz(e.target.value)} placeholder="Africa/Nairobi" />
          </div>
          <div className="grid gap-2">
            <Label>Locale</Label>
          ...
          <div className="grid gap-2">
            ...
            <Input type="number" min={30} value={retention} onChange={(e) => setRetention(Number(e.target.value))} />
            <p className="text-xs text-muted-foreground">Default 2555 (7 years). Applies to case data per your compliance policy.</p>
          </div>
...
function BrandingTab({ orgId, org, onSaved }: { orgId: string; org: OrgRow; onSaved: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: () => update({ data: { orgId, branding: { primary_color: primary, logo_url: logo, tagline } } }),
    onSuccess: () => { toast.success("Branding updated"); onSaved(); },
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      <CardContent>
        <form className="grid gap-4 sm:grid-cols-2" onSubmit={(e) => { e.preventDefault(); mut.mutate(); }}>
          <div className="grid gap-2">
            ...
            <div className="flex items-center gap-3">
              <input type="color" value={primary} onChange={(e) => setPrimary(e.target.value)} className="h-10 w-14 cursor-pointer rounded border" />
          ...
          <div className="grid gap-2 sm:col-span-2">
            <Label>Tagline</Label>
            <Input value={tagline} onChange={(e) => setTagline(e.target.value)} placeholder="Care, when it matters" />
          </div>
          <div className="sm:col-span-2 flex items-center gap-4 rounded-md border border-border bg-card p-4">
            <div className="grid h-12 w-12 place-items-center rounded-lg text-white font-display text-lg font-bold" style={{ backgroundColor: primary }}>
          ...
          <div className="sm:col-span-2">
            <Button type="submit" disabled={mut.isPending}>{mut.isPending ? "Saving…" : "Save branding"}</Button>
          </div>
        </form>
      </CardContent>
    </Card>
...
const DEFAULT_TERMS = {
  ...
  counselor: "Counselor", counselors: "Counselors",
  session: "Session", sessions: "Sessions",
  case: "Case", cases: "Cases",
};

function TerminologyTab({ orgId, org, onSaved }: { orgId: string; org: OrgRow; onSaved: () => void }) {
  const update = useServerFn(updateOrganizationSettings);
  ...
  const mut = useMutation({
    ...
    onSuccess: () => { toast.success("Terminology updated"); onSaved(); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        ...
        <CardDescription>Rename core concepts to match your institution (e.g. "Students" instead of "Clients").</CardDescription>
      </CardHeader>
      <CardContent>
        <form className="grid gap-4 sm:grid-cols-2" onSubmit={(e) => { e.preventDefault(); mut.mutate(); }}>
          ...
          <div className="sm:col-span-2 flex gap-2">
            <Button type="submit" disabled={mut.isPending}>{mut.isPending ? "Saving…" : "Save terminology"}</Button>
            <Button type="button" variant="outline" onClick={() => setTerms(DEFAULT_TERMS)}>Reset to defaults</Button>
          </div>
        </form>
      </CardContent>
    </Card>
...
const ROLE_OPTIONS: AppRole[] = ["org_admin", "counselor", "clinical_supervisor", "org_staff", "client"];
...
function MembersTab({ orgId }: { orgId: string }) {
  ...
  const mStatus = useMutation({
    mutationFn: (v: { userId: string; status: MemberStatus }) => setStatus({ data: { orgId, ...v } }),
    ...
    <Card className="mt-6">
      ...
      <CardContent>
        {q.isLoading ? (
          ...
          <div className="text-sm text-muted-foreground">No members yet — invite people from the Invitations tab.</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                  ...
                  <TableRow key={m.user_id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Select onValueChange={(v) => mAssign.mutate({ userId: m.user_id, role: v as AppRole })}>
                          <SelectTrigger className="h-8 w-[160px]"><SelectValue placeholder="Add role" /></SelectTrigger>
                          <SelectContent>
...
function InvitationsTab({ orgId }: { orgId: string }) {
  ...
  const q = useQuery({ queryKey: ["invitations", orgId], queryFn: () => list({ data: { orgId } }) });
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        ...
        <CardDescription>Invite people to join this organization. They accept via a secure link.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <form className="grid gap-3 sm:grid-cols-[1fr_200px_auto]" onSubmit={(e) => { e.preventDefault(); mCreate.mutate(); }}>
          <Input type="email" required placeholder="person@example.com" value={email} onChange={(e) => setEmail(e.target.value)} />
        ...
        {q.isLoading ? <div className="text-sm text-muted-foreground">Loading…</div> : (
          <Table>
            ...
            <TableBody>
              {(q.data ?? []).length === 0 && (
                <TableRow><TableCell colSpan={5} className="text-center text-sm text-muted-foreground">No invitations yet.</TableCell></TableRow>
              )}
              {(q.data ?? []).map((inv) => {
                const state = inv.revoked_at ? "revoked" : inv.accepted_at ? "accepted" : new Date(inv.expires_at) < new Date() ? "expired" : "pending";
                const link = `${window.location.origin}/invite/${inv.token}`;
                return (
                  <TableRow key={inv.id}>
                    ...
                    <TableCell>
                      <Badge variant={state === "pending" ? "outline" : state === "accepted" ? "default" : "destructive"}>
                        {state}
                      </Badge>
                    </TableCell>
...
function AuditTab({ orgId }: { orgId: string }) {
  ...
  const q = useQuery({ queryKey: ["audit", orgId], queryFn: () => list({ data: { orgId, limit: 200 } }) });

  return (
    <Card className="mt-6">
      <CardHeader>
        <CardTitle className="text-base">Audit log</CardTitle>
        <CardDescription>Every mutation in this organization is recorded here (append-only).</CardDescription>
      </CardHeader>
      <CardContent>
        {q.isLoading ? <div className="text-sm text-muted-foreground">Loading…</div> : (
          <Table>
            ...
            <TableBody>
              ...
              {(q.data ?? []).map((e) => (
                <TableRow key={e.id}>
                  ...
                  <TableCell className="text-sm">{e.entity_type}{e.entity_id ? ` · ${e.entity_id.slice(0,8)}…` : ""}</TableCell>
                  <TableCell className="text-xs font-mono text-muted-foreground">{e.actor_id?.slice(0, 8)}…</TableCell>
                  <TableCell className="max-w-xs truncate text-xs text-muted-foreground">{e.diff ? JSON.stringify(e.diff) : ""}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
